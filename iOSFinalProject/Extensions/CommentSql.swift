...
extension Comment{
    ...
    static func createTable(database: OpaquePointer?)  {
        ...
        let res = sqlite3_exec(database, "CREATE TABLE IF NOT EXISTS COMMENTS (ID TEXT PRIMARY KEY, COMMENT TEXT, IMAGE TEXT, UID TEXT, ACTIVE INTEGER)", nil, nil, &errormsg)
        if(res != 0){
    ...
    static func getAll(database: OpaquePointer?, prefix:String)->[Comment]?{
        ...
        if (sqlite3_prepare_v2(database,"SELECT * from COMMENTS WHERE id LIKE '\(prefix)%';",-1,&sqlite3_stmt,nil)
            ...
            while(sqlite3_step(sqlite3_stmt) == SQLITE_ROW){
                ...
                let isActive = (sqlite3_column_int(sqlite3_stmt,4) != 0)

                data.append(Comment(_id: id, _text: text, _imageUrl: imageUrl, _userUid: userUid, _isActive: isActive))
            }
        }
    ...
    static func addNew(database: OpaquePointer?, comment:Comment){
        var sqlite3_stmt: OpaquePointer? = nil
        if (sqlite3_prepare_v2(database,"INSERT OR REPLACE INTO COMMENTS(ID, COMMENT, IMAGE, UID, IMAGE) VALUES (?,?,?,?,?);",-1, &sqlite3_stmt,nil) == SQLITE_OK){
            ...
            sqlite3_bind_int(sqlite3_stmt, 5, Int32(isActive));
